...
function getPagesInfo({ defaultEntries, scanDir, scanFile, specialPageNames, ignorePageNames }: MpaOptions): PageInfo {
  const allFiles = fg.sync(`${scanDir}/${getSpecialPageNames(specialPageNames)}/${scanFile}`.replace('//', '/'), {
...
export function getMPAIO(root: string, options: MpaOptions) {
  const { scanFile, filename, buildPrefixName } = options;
  const pages = getPagesInfo(options);
  const input: Record<string, string> = {};
  Object.keys(pages).map((key) => {
    input[buildPrefixName + key] = path.resolve(root, scanFile2Html(pages[key].entry, scanFile, filename));
  });
